    ...
    for (int i = 0; i < CFArrayGetCount(lines); i++)
    {
        ...
        for (int j = 0; j < CFArrayGetCount(runs); j++)
        {
            ...
            if (richTextRun.drawSelf)
            {
                ...
                CGRect runRectDraw = CGRectMake(runPointX, runPointY-3, runWidth, runHeight);
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (self.delegage && [self.delegage respondsToSelector:@selector(richTextView: touchBeginRun:)])
    {
        ...
        NSArray *arr=[self.runRectDictionary allKeys];
        for (int i=0;i<[arr count]; i++)
        {
            CGRect rect = [[arr objectAtIndex:i] CGRectValue];
            if(CGRectContainsPoint(rect, runLocation))
            {
                self.touchRun = [self.runRectDictionary objectForKey:[arr objectAtIndex:i]];
                [weakSelf.delegage richTextView:weakSelf touchBeginRun:[self.runRectDictionary objectForKey:[arr objectAtIndex:i]]];
                self.Y=!self.Y;
            }
        }
        if (!self.Y)
        {
            __weak TQRichTextView *weakSelf = self;
            [weakSelf.delegage richTextView:weakSelf];
        }
        else
        {
            self.Y=NO;
...
+ (NSMutableAttributedString *)createAttributedStringWithText:(NSMutableString *)text font:(UIFont *)font lineSpace:(CGFloat)lineSpace
{
