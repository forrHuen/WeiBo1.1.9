...
-(id)initWithDictionary:(NSDictionary *)comments
{
    if (self=[super init])
    {
        ...
        self.source=str;
        if ([[comments objectForKey:@"reply_comment"]allKeys].count==0)
        {
            self.reply_text=[[comments objectForKey:@"status"] objectForKey:@"text"];
        }
        else
        {
            self.reply_text=[NSString stringWithFormat:@"回复我的评论:%@",[[comments objectForKey:@"reply_comment"] objectForKey:@"text"]];
        }
